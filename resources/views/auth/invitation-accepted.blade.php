...
<head>
    ...
    <title>Invitation Confirmed — AttendFlow</title>
...
<body class="h-full bg-slate-950 flex flex-col justify-center items-center p-4">
    <div class="w-full max-w-md bg-slate-900 border border-white/10 rounded-3xl p-8 shadow-2xl backdrop-blur-xl text-center space-y-6 animate-fadeIn">
        @if($success)
            ...
            <div>
                <span class="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 tracking-wider uppercase">CONFIRMATION RECEIPT VERIFIED</span>
                <h1 class="text-2xl font-extrabold text-white mt-3">Workspace Confirmed!</h1>
            ...
            <div class="p-4 rounded-2xl bg-slate-950 border border-white/5 text-left space-y-2 text-xs">
                <div class="flex justify-between border-b border-white/5 pb-2">
                    ...
                    <span class="font-bold text-blue-400">{{ $organization->name ?? 'Workspace' }}</span>
                </div>
                <div class="flex justify-between border-b border-white/5 pb-2">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-bold text-emerald-400">✅ Confirmed</span>
                </div>
            </div>
        ...
        @else
            ...
            <h1 class="text-2xl font-extrabold text-white">Confirmation Error</h1>
            <p class="text-slate-400 text-sm">{{ $message }}</p>
            <a href="{{ route('login') }}" class="block w-full py-3 px-4 bg-white/10 text-white font-bold rounded-xl text-sm hover:bg-white/20 transition-all">
                Proceed to Login
            </a>
        @endif
    </div>
