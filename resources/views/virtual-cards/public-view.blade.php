        ...
        <div id="virtual-id-card-element" class="relative overflow-hidden rounded-3xl bg-gradient-to-b from-[#0e1628] via-[#0a0f1d] to-[#060a14] border border-blue-500/30 ring-1 ring-white/10 id-card-glow p-5 sm:p-6 space-y-5">
            ...
            <div class="flex items-start justify-between border-b border-white/10 pb-3.5 relative z-10 gap-3">
                <div class="flex items-start gap-2.5 min-w-0 flex-1">
                    @if($card->institution_logo_url)
                        <img src="{{ $card->institution_logo_url }}" alt="Logo" class="w-10 h-10 object-contain rounded-xl bg-white/5 p-1 border border-white/10 shrink-0 shadow-sm mt-0.5">
                    @else
                    ...
                    <div class="flex-1 min-w-0 space-y-1">
                        <div class="text-[11px] sm:text-xs font-black uppercase tracking-wider text-blue-400 font-sans leading-tight">
                            {{ $card->organization ? $card->organization->name : 'Federation of African Law Students' }}
                        </div>
